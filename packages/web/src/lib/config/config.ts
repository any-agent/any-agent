...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.ANYAGENT_CONFIG) {
      result = mergeDeep(result, await loadFile(Flag.ANYAGENT_CONFIG))
      log.debug("loaded custom config", { path: Flag.ANYAGENT_CONFIG })
    }

    if (Flag.ANYAGENT_CONFIG_CONTENT) {
      result = mergeDeep(result, JSON.parse(Flag.ANYAGENT_CONFIG_CONTENT))
      log.debug("loaded custom config from ANYAGENT_CONFIG_CONTENT")
    }
    ...
    if (Flag.ANYAGENT_CONFIG_DIR) {
      directories.push(Flag.ANYAGENT_CONFIG_DIR)
      log.debug("loading config from ANYAGENT_CONFIG_DIR", { path: Flag.ANYAGENT_CONFIG_DIR })
    }
    ...
    if (Flag.ANYAGENT_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ANYAGENT_PERMISSION))
    }
      ...
      lsp: z
        ...
        .refine(
          (data) => {
            ...
            return Object.entries(data).every(([_, config]) => {
